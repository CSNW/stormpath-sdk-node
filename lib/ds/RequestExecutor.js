...
RequestExecutor.prototype.execute = function executeRequest(req, callback, num_times) {
  ...
  if (in_progress_gets[options.uri] && options.method == 'GET') {
    console.log('additional hit with in-progress GET: ' + options.uri);
    return in_progress_gets[options.uri].callbacks.push(callback);
  }
  ...
  */
  ...
  if (options.method == 'GET')
    in_progress_gets[options.uri] = {callbacks: [callback]};
  ...
  request(options, function onRequestResult(err, response, body) {
    ...
    var callbacks;
    if (options.method == 'GET') {
      callbacks = in_progress_gets[options.uri].callbacks;
      delete in_progress_gets[options.uri];
      if (callbacks.length > 1)
        console.log('Sending response to ' + options.uri + ' to ' + callbacks.length + ' callbacks');
    }
    else {
      callbacks = [callback];
    }
